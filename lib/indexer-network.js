...
export function providerForUrl(url) {
  ...
  if (hostname.includes('animetosho.')) return 'animetosho'
  if (hostname === 'api.anisearch.org' || hostname.endsWith('.anisearch.org')) return 'anisearch'
  if (hostname === 'nekobt.to' || hostname.endsWith('.nekobt.to')) return 'nekobt'
  if (hostname === 'releases.moe' || hostname.endsWith('.releases.moe')) return 'seadex'
  return hostname
}
...
export function createIndexerNetwork({
  ...
  function noteFailure(url, error) {
    ...
    const isDirectStorage = state.host.endsWith('.craftervault.com') || state.host.endsWith('.anibatchddl.com') || state.host === 'craftervault.com' || state.host === 'anibatchddl.com'
    const immediate = ['RATE_LIMITED', 'SERVICE_UNAVAILABLE', 'INVALID_RESPONSE'].includes(error.code) || error.status === 403 && !isDirectStorage
    // A single inaccessible video may be file-specific. Repeated 403s still
    // indicate a shared outage; do not keep probing thousands of files.
    const transient = ['NETWORK_ERROR', 'TIMEOUT'].includes(error.code) || error.status >= 500 || error.status === 403
  ...
  async function scheduledFetch(url, options = {}) {
    ...
    try {
      for (let redirects = 0; redirects <= 5; redirects++) {
        ...
        let response
        timer = setTimeout(() => controller.abort(new DOMException('Plazo de transferencia agotado', 'TimeoutError')), requestTimeout)
        try {
          const pendingResponse = Promise.resolve().then(() => {
            signal.throwIfAborted()
            return fetchFn(currentUrl, { ...fetchOptions, signal, timeout: requestTimeout, redirect: 'manual' })
          })
          ...
          if (response.status === 429 || response.status === 503 || response.status === 403) {
            cancelBody(response)
            const isDirectStorage = state.host.endsWith('.craftervault.com') || state.host.endsWith('.anibatchddl.com') || state.host === 'craftervault.com' || state.host === 'anibatchddl.com'
            if (isDirectStorage && response.status === 403) {
              throw new IndexerNetworkError(`HTTP 403 en archivo de ${state.host}`, {
                ...context, status: 403, code: 'HTTP_ERROR'
              })
            }
          ...
          if (requiredStatus && response.status !== requiredStatus) {
            cancelBody(response)
            throw new IndexerNetworkError(`Se esperaba HTTP ${requiredStatus}, llegó HTTP ${response.status}`, {
              ...context, code: 'RANGE_UNAVAILABLE', status: response.status
            })
          }
