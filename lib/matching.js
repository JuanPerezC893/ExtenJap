...
export function isBatchTorrent(item) {
  ...
  if (typeof item.num_files === 'number') {
    if (item.num_files === 1) return false
    if (item.num_files > 1) return true
  }
  const title = `${item.title || ''} ${item.torrent_name || ''}`
  if (/\b(batch|complete\s+(?:series|season)|season\s*\d*\s*complete)\b/i.test(title)) return true
  // A spaced title/episode separator is not a range: "Season 2 - 01"
  // and "86 - 01" are individual episodes. Ambiguous names go through
  // metadata validation, which remains authoritative about file count.
  return /(?:^|[\s[(])\d{1,3}[-~]\d{1,3}(?=[\s\])]|$)/.test(title)
    || /(?:[[(]|\s-\s+)\d{1,3}\s*[-~]\s*\d{1,3}(?=[\s\])]|$)/.test(title)
}
...
export const extractGroup = text => {
...
export function rankCandidates(items, targetEp, siblingEps = []) {
  ...
  const targetFn = fileName(targetEp)
  const wantedRes = resolution(targetFn) || String(targetEp.resolution || '')
  const targetGroup = targetEp.group?.toLowerCase() || extractGroup(targetFn) || (targetEp.quality ? extractGroup(targetEp.quality) : null)
  ...
  const valid = items.filter(item => {
    ...
    const candGroup = extractGroup(item.title)
    if (targetGroup && candGroup && targetGroup !== candGroup) return false
    return true
  })
