...
try {
  ...
  console.warn('[ProxyPool] undici no disponible o incompatible con este entorno:', err.message)
}
...
export class ProxyPool {
  constructor(options = {}) {
    this.enabled = (options.enabled ?? false) && Boolean(ProxyAgent)
  ...
  async fetch(url, options = {}, retries = this.maxRetries) {
    if (!this.enabled || !undiciFetch || !ProxyAgent) {
