...
export function buildSiblingQueries(ep, siblings = []) {
  ...
  for (const s of siblings) {
    const sEpNum = Number(s.episode)
    if (!Number.isFinite(sEpNum) || sEpNum === epNum) continue
    const sFn = s.sourceFileName || s.fileName || ''
    if (!sFn) continue

    const patterns = [
      { from: new RegExp(`S(\\d{1,2})E0*${sEpNum}\\b`, 'i'), to: (m, season) => `S${season}E${epStr}` },
      { from: new RegExp(`\\bE0*${sEpNum}\\b`, 'i'), to: `E${epStr}` },
      { from: new RegExp(`(\\s-\\s*)0*${sEpNum}\\b`, 'i'), to: `$1${epStr}` },
      { from: new RegExp(`\\[0*${sEpNum}\\]`, 'i'), to: `[${epStr}]` },
      { from: new RegExp(`_0*${sEpNum}_`, 'i'), to: `_${epStr}_` },
      { from: new RegExp(`(\\s)0*${sEpNum}(\\s)`, 'i'), to: `$1${epStr}$2` }
    ]

    for (const pat of patterns) {
      if (pat.from.test(sFn)) {
        const transformed = sFn.replace(pat.from, pat.to)
        // 1. Extraer la raíz limpia hasta el número de episodio (evita arrastrar el subtítulo/nombre del episodio hermano)
        const matchUpToEp = transformed.match(new RegExp(`^(.*?\\b(?:S\\d{1,2}E${epStr}|E${epStr}|-\\s*${epStr}|\\[${epStr}\\]|_${epStr}_|\\s${epStr}\\b))`, 'i'))
        if (matchUpToEp) {
          const stem = cleanReleaseFileName(matchUpToEp[1])
            .replace(/\[[a-f0-9]{8}\]/gi, '')
            .replace(/[._\-]/g, ' ')
            .replace(/\s+/g, ' ')
            .trim()
          if (stem && stem.length > 5 && !seen.has(stem)) {
            seen.add(stem)
            queries.push({ query: stem, priority: 'sibling', provider: 'all' })
          }
        }

        // 2. Consulta con nombre completo
        const clean = cleanReleaseFileName(transformed)
          .replace(/\[[a-f0-9]{8}\]/gi, '')
        ...
        if (clean && clean.length > 5 && !seen.has(clean)) {
          seen.add(clean)
          queries.push({ query: clean, priority: 'sibling', provider: 'all' })
        }
      }
    }
  }
...
export function buildSearchQueries(series, ep, siblings = []) {
  const epNum = Number(ep.episode)
  ...
  // File naming often carries the release title/season absent from catalogue aliases.
  const stemInput = cleanFn.replace(/\(\d{4}\)/g, '').replace(/^\[[^\]]+\]\s*/, '')
  const stemMatch = stemInput.match(/^(.*?\bS\d{1,2}E\d{1,3}(?:v\d+)?\b|.*?\s-\s*\d+(?:\.\d+)?(?:v\d+)?\b)/i)
  if (stemMatch) {
    const stem = stemMatch[0].replace(/[:\-!?,._~"'()]/g, ' ').replace(/\s+/g, ' ').trim()
    if (stem) queries.push({ query: stem, priority: 'release_stem', provider: 'all' })
  }
  const titleFromFile = stemMatch?.[0].replace(/\bS\d{1,2}E\d{1,3}.*$/i, '').replace(/\s-\s*\d.*$/, '').trim()
  const broadTitle = (titleFromFile || cleanTitle).replace(/[:\-!?,._~"'()]/g, ' ').replace(/\s+/g, ' ').trim()
  if (broadTitle.length >= 3) queries.push({ query: broadTitle, priority: 'series_broad', provider: 'all' })

  // 2. Si hay hermanos verificados, la consulta derivada del hermano va inmediatamente después de CRC
  for (const sq of buildSiblingQueries(ep, siblings)) {
    queries.push(sq)
  }
